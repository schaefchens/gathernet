...
import { dmKey, markRead } from '../stores/read-state.ts'
...
function ChatScreen() {
  ...
  // An open conversation is a read conversation: clear its unread mark as messages arrive.
  const newestSentAt = messages[messages.length - 1]?.sentAt ?? 0
  useEffect(() => {
    if (newestSentAt) markRead(dmKey(friendId), newestSentAt)
